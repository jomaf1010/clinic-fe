...
const local = reactive({
  ...
  lmp: (props.triage.specialty_data as Record<string, string | null> | null | undefined)?.lmp ?? null,
})
...
watch(
  ...
  (t) => {
    ...
    local.notes = t.notes
    local.lmp = (t.specialty_data as Record<string, string | null> | null | undefined)?.lmp ?? null
...
<template>
  <div class="flex flex-col gap-6">
    ...
    <!-- LMP (OB-GYN specific) -->
    <div class="flex flex-col gap-2">
      <Label for="lmp_date" class="flex items-center gap-1.5">
      ...
      <Input
        ...
        @update:model-value="(v: string | number) => { local.lmp = String(v) || null }"
        @blur="onBlur"
      />
    </div>
